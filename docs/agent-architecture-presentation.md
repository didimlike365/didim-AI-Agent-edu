...
이 Agent는 의료 질문을 일반 QA처럼 바로 답하지 않고, 증상 분석, 질문 대상 확인, 응급 확인, 검색 같은 tool을 먼저 실행해 안전성과 근거를 확보한 뒤 최종 답변을 생성하도록 설계된 Agent다.
...
현재 Agent는 질문 유형에 따라 아래 tool들을 조합해서 사용한다.
...
- 첫 번째 LLM: 내부 agent를 통해 어떤 tool을 쓸지 결정
...
- `app/agents/search_agent.py`
  - LangGraph 기반으로 전체 흐름 제어
  - 내부 agent가 tool schema를 바인딩하고 1차 tool 실행 수행
  - tool 실행 결과 조합
