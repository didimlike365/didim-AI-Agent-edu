...
검색형 tool은 Elasticsearch 원문을 그대로 전달하지 않고, 먼저 축약된 컨텍스트로 재구성합니다. 이 압축 단계는 별도 저비용 모델을 사용하거나, 실패 시 규칙 기반 fallback으로 짧은 근거만 남깁니다.
...
- 검색 결과에서 필요한 필드만 남기고 본문 길이를 줄입니다.
- 압축 모델이 활성화되어 있으면 요약/핵심 포인트 형태로 재구성합니다.
- 압축이 실패하면 문서별 핵심 근거만 담은 fallback 문자열을 반환합니다.

#### 출력 예시

```text
[의학 검색 요약]
사용자 질문: latent TB infection diagnosis에 대해 설명해줘
요약: 잠복결핵감염은 증상이 없을 수 있지만 검사와 평가를 통해 확인한다.
핵심 포인트:
- 잠복결핵감염은 활동성 결핵과 구분된다.
- 진단은 검사 결과와 임상 판단을 함께 본다.
참고 문서:
- 문서 1, c_id=25276_3, year=2000, source_spec=who
```
...
- 별도 병원 인덱스를 쓰지 않습니다.
- LLM이 선택한 `query`와 `region` 인자를 그대로 사용해 `edu-collection`을 검색합니다.
- 지역/병원 관련 문맥도 동일하게 압축해서 핵심 포인트만 남깁니다.
...
```text
[지역/병원 검색 요약]
사용자 질문: 서울 근처 결핵 관련 병원 정보도 같이 알려줘
요약: 검색 문서에는 서울권 결핵 진료 관련 기관 정보가 일부 언급된다.
핵심 포인트:
- 지역명과 질환 키워드가 함께 등장한 문서를 우선 압축한다.
참고 문서:
- 문서 1, c_id=25276_3, year=2000, source_spec=who
```
