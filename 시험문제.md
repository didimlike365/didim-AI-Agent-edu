...
**Bug 1**: _checkpointer = MemorySaver() #재사용 가능하도록 모듈레벨로 이동  
**Bug 2**: @tool  # bug 2
    def search_symptom(self, symptom: str) -> str:
        """증상으로 질병 정보를 검색합니다."""
        return f"{symptom} 관련 질병 정보" #클래스 밖의 일반 함수로 이동  
**Bug 3**: response_format=ToolStrategy(ChatResponse)  # 오타 수정 fromat -> format
...
**문제점**: process_query()가 호출될 때마다 새 InMemorySaver()를 만들고, 그걸로 새 agent를 생성. 그러면 같은 thread_id를 보내도 이전 턴의 상태를 같은 checkpointer에서 찾을 수 없어 멀티턴 대화 연속성이 사라짐. LangGraph/LangChain의 short-term memory는 checkpointer에 저장되고, 이 저장소는 thread_id를 키로 상태를 읽고 씁니다. thread_id 기반으로 대화를 유지하려면 동일한 checkpointer 인스턴스를 재사용 해야함
**수정 코드**:
...
**(1)** 문제점: 버전에 따라 존재하지 않을수 있음 → 올바른 경로: from langchain.agents import ToolStrategy  
**(2)** 문제점: deprecated 경로 → 올바른 경로: from langchain_core.tools import tool  
**(3)** 상태: 정상  
**(4)** 상태: 정상  
**(5)** 상태: 정상
...
**답변** (3~5문장):공식적으로 권장되는 쪽은 response_format=ToolStrategy(ChatResponse) 또는 스키마 타입 직접 전달입니다. 이유는 세 가지입니다.<br>첫째, LangChain이 구조화 출력을 위해 제공하는 공식 API이기 때문입니다.<br>둘째, 결과를 스키마에 맞게 검증하고 예측 가능한 형태로 반환해 후속 코드가 다루기 쉽습니다.<br>셋째, 최신 v1 문서 기준으로 구조화 출력은 에이전트 메인 루프에 통합되어 있어, 예전처럼 “최종 응답용 fake tool을 따로 만드는 우회 패턴”보다 더 표준적이고 안정적입니다
